...
# Interpret platform
if(grepl('pulldown', project)) {
	platform = 'pulldown'

	# Interpret macs2 or PePr peaks
	if(grepl('PePr', project)) {
		peak_type = 'PePr'
	} else if (grepl('macs2', project)) {
		peak_type = 'macs2'
	}
} else if (grepl('bisulfite', project)) {
	platform = 'bisulfite'
}

# Interpret mark
# This order matters very much!
if(grepl('mc_hmc', project)) {
	mark = 'mc_hmc'
} else if (grepl('hmc', project)) {
	mark = 'hmc'
} else if (grepl('mc', project)) {
	mark = 'mc'
}
